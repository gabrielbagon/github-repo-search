...
export default function Home() {
  ...
  const clampPerPage = (v: number) => (ALLOWED_PER_PAGE.includes(v as any) ? v : 10);
  const clampPage = (v: number) => (Number.isFinite(v) && v >= 1 ? v : 1);

  // Estado inicial sincronizado com URL + prefs (sem efeito extra)
  const initial = useMemo(() => {
    if (typeof window === "undefined") {
      return { query: "", sort: "best" as const, order: "desc" as const, perPage: 10, page: 1, language: "" };
    }
 ...
 {/* Navbar com toggle do PAT */}
      ...
      <main className="flex flex-col gap-[32px] row-start-2 items-center sm:items-start w-full max-w-[960px]">
        ...
        <div className="mt-1 text-xs text-neutral-500" aria-live="polite">
          {token ? "Autenticação: ativa" : "Autenticação: anônima"} • {rate.remaining ?? 0}/{rate.limit ?? 0} requisições restantes
        </div>
        ...
        {/* ERROR */}
        {error && (
          <section role="alert" className="mt-4 w-full rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-red-200">
            <p className="text-sm">{error}</p>
            {errorCode === 403 && (
              <p className="mt-2 text-xs text-red-300">
                Dica: a Search API do GitHub é limitada para usuários anônimos. Adicione um <strong>PAT</strong> para elevar seu limite.
              </p>
            )}
          </section>
        )}
        ...
        <section className="mt-4 grid gap-3 w-full">
          ...
          {/* Lista */}
          {!loading && hasItems && data?.items?.map((repo) => <RepoCard key={repo.id} repo={repo} />)}
